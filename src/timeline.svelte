<script lang="ts">
  ...
  function parseColor(item: PlanItem) {
  ...
  <div id="day-planner-timeline-container">
    ...
    <div class="events">
      {#each summary.items as item, i}
        <div
          class="event_item {shortClass(
            item
          )} {pastClass(item)}"
          style="height: {item.durationMins *
            timelineZoomLevel}px;background-color: {parseColor(item)[1] ?? ''};"
        >
          <div class="event_item_contents">
            <div
              class="ei_Dot {item === summary.current ? 'dot_active' : ''}"
            />
            <div class="ei_Title" style="color: {parseColor(item)[2] ?? ''};">{item.rawTime}</div>
            <div class="ei_Copy" style="color: {parseColor(item)[2] ?? ''};">{parseColor(item)[0] ?? ""}</div>
          </div>
        </div>
      {/each}
    </div>
